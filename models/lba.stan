...
parameters {
    ...
    real<lower=0> v_false_mean;
    real<lower=0,upper=1> v_false_sd;
    real<lower=0.1,upper=1> tau_mean;
    real<lower=0,upper=1> tau_sd;

    real v_true_pre_mean;
    real<lower=1> v_true_pre_sd;
...
transformed parameters {
     ...
     A = A_mean + A_sd*A_raw;
     B = B_mean + B_sd*B_raw;
     v_false = v_false_mean + v_false_sd*v_false_raw;
     v_true_pre = v_true_pre_mean + v_true_pre_sd*v_true_pre_raw;

     //calculate means for change parameters
     dvt_anodal_mean = COVS * COEFS_anodal;
     dvt_cathodal_mean = COVS * COEFS_cathodal;
     dvt_sham_mean = COVS * COEFS_sham;
...
model {
     ...
     RT ~ lba(A[subject],B[subject],v_false[subject],s,tau[subject],v_true);
}
