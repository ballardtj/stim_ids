    ...
    for ( i in 1:ni ) if ( !all(unlist(lapply(pp[[1]][[i]],is.null))) ) {
      ...
      if ( style=="pdf" ) {
        ...
        for (j in inj) {
          ...
          if ( fit.points && show.fits && !is.null(attr(pp,"dpqs")) ) for (k in 1:length(attr(pp,"dpqs"))) {
            ppk <- attr(pp,"dpqs")[[k]]
            if (no.facs) {
              y <- as.numeric(names(ppk[[style]][[ j ]][[1]])) 
              x <- ppk[[style]][[ j ]][[1]]
            ...
            if ( (!any(is.na(percentiles)) & !is.null(ppk)) ) 
  ...
  # from the save.simulation output of post.predict
{
  ...
  out <- list(pp.df, RT.df,get.stat(mean,data,sim,tapplyvec),
    get.stat(sd,data,sim,tapplyvec))
  names(out) <- c("pps", "RTs","MeanRTs","SDRTs")
  out
}
...
plot_SS_srrt.dmc <- function(data,sim=NULL,minN=1,ci=.99,sim.line=FALSE,
  ...
  xlabels=NULL,unique.breaks=TRUE)
